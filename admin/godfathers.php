    ...
    <section class="row">
      <div class="mx-auto w-50">
        <?php
          ...
          $count_query = $database->query('SELECT COUNT(*) AS total_inputs FROM parrain');
          ...
          $get_query = $database->query("SELECT * FROM parrain LIMIT $per_page OFFSET $start");
        ?>
          ...
          <table class="table table-striped table-bordered table-responsive">
            ...
            <tbody>
              <?php $i = $start; while($row = $get_query->fetch()){
                $i++;
                $user = getUserById($database, $row['User_idUser']);
                if(!$user) continue;
                $step_query = $database->query('SELECT Step_idStep FROM parrain_has_step WHERE Parrain_idParrain = '.$row['idParrain'].' AND readyUp = 0');
                $step = $step_query->fetch();
                $step = !$step['Step_idStep'] ? 'Indéfini' : $step['Step_idStep'];
              ?>
              <tr>
                <td><?= $i > 9 ? $i : '0'.$i ?></th>
                <td><?= $user['lastName'] == NULL ? "Non défini" : $user['lastName'] ?></td>
                <td><?= $user['firstName']  == NULL ? "Non défini" : $user['firstName'] ?></td>
                <td><?= $user['sex'] == "M" ? "Masculin" : $user['sex'] == "F" ? "Féminin" : "Non défini" ?></td>
                <td><?= ($step-1) ?></td>
                <td>
