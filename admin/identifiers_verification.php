<?php
  ...
  if (!empty($_POST['login']) && !empty($_POST['password'])) {
    ...
    $data = $query->fetch();
    if (!$data) header('Location: http://localhost/Web_Projects/HappyFoodWebsite/admin/index.php?status=0');
    ...
    else header('Location: http://localhost/Web_Projects/HappyFoodWebsite/admin/index.php?status=0');
  }
  else header('Location: http://localhost/Web_Projects/HappyFoodWebsite/admin/index.php?status=0');
