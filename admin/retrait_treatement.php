<?php
  ...
  if (isset($_POST['parrainId'], $_POST['stepId'])) {
    ...
    if ($upated>0) {
      ...
      header('Location: http://localhost/HappyFoodWebsite/admin/compensations.php?lastName='.$user['lastName'].'&firstName='.$user['lastName'].'&step='.$_POST['stepId']);
    }
    else {
      header('Location: http://localhost/HappyFoodWebsite/admin/compensations.php?error=0');
    }
  }
  else {
      header('Location: http://localhost/HappyFoodWebsite/admin/compensations.php?error=0');
